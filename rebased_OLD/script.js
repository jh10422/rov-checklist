...
class ROVChecklistApp {
    ...
    checkCompletion(checkedCount, totalVisible) {
        // Override
    }
...
class AbortMissionChecklist extends TroubleshootingApp {
    ...
    checkTabCompletion() {
        ...
        // Show "Proceed to next page" messages for Vehicle and Tether tabs
        if (this.currentPage === 1) { // Vehicle tab
            const vehicleProceed = document.getElementById('vehicle-proceed');
            if (vehicleProceed) {
                vehicleProceed.style.display = isChecklistTabComplete ? 'table-row' : 'none';
            }
        } else if (this.currentPage === 2) { // Tether tab
            const tetherProceed = document.getElementById('tether-proceed');
            if (tetherProceed) {
                tetherProceed.style.display = isChecklistTabComplete ? 'table-row' : 'none';
            }
        }
    ...
    updateProgress() {
        ...
        let visibleCheckableItems, checkedVisibleItems, totalVisible, checkedCount;
        
        if (this.currentPage === 3) { // Control Station tab - special handling
            // For Control Station, we need to count visible conditional items properly
            visibleCheckableItems = currentPageElement.querySelectorAll('.checklist-item:not(.is-question):not(.conditional), .checklist-item:not(.is-question).conditional[style*="table-row"]');
            checkedVisibleItems = [...visibleCheckableItems].filter(item => item.classList.contains('checked'));
            
            totalVisible = visibleCheckableItems.length;
            checkedCount = checkedVisibleItems.length;
            
            // Check if Control Station is complete (either YES or NO path finished)
            ...
            // For other tabs, use normal logic
            visibleCheckableItems = currentPageElement.querySelectorAll('.checklist-item:not(.is-question):not(.conditional), .checklist-item:not(.is-question).conditional[style*="table-row"]');
            checkedVisibleItems = [...visibleCheckableItems].filter(item => item.classList.contains('checked'));
    ...
    updateFooter() {
        // Only show "GO TO HOME" when on the final tab (Control Station) AND it's complete
        if (this.currentPage === 3) { // Control Station tab
            const csComplete = document.getElementById('cs-yes-complete')?.style.display === 'table-row' || 
                               document.getElementById('cs-no-complete')?.style.display === 'table-row';
            ...
            // For tabs 1-3, always show pager controls, never show "GO TO HOME"
            if (this.pagerControls) this.pagerControls.style.display = 'flex';
            if (this.nextChecklistBtn) this.nextChecklistBtn.style.display = 'none';
        }
    }
